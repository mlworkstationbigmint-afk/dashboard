...
- **Accent = orange**: `primaryColor` is now `#EE4E24` (ACCENT orange) in config.toml — it natively drives **primary buttons (Sign in / Log out / active nav) + tab highlights + segmented selected state** orange. The **brand topbar stays blue** because it uses the `PRIMARY` (#024CA1) constant in `theme.py` CSS, *not* `primaryColor`. The `!important` orange tab/segmented overrides in `theme.py` (`stBaseButton-segmented_controlActive` / `tab-highlight` selectors) are now belt-and-suspenders — kept so the selected-tab *label* is orange and as a fallback, but the orange no longer depends on them matching a given Streamlit version. (Was previously blue buttons + CSS-only orange tabs; flipped 2026-06-26.)
